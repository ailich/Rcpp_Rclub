...
-   **R is a high level "interpreted" programming language**. This makes it fast to develop code, but slower to execute since it is run line by line.

-   **C++ is a low level "compiled" language**. It takes more time to write code but it will execute faster. This is because the code must be compiled before execution. A compiler will figure out the optimal order to execute code rather than running it line by line.
...
In C++ code you must define the types and dimensions of your variables/outputs and cannot reassign them (you can update them).
...
There are many ways to create matrices. Below are some ways to create C++ vectors with the equivalent R code. `NumericMatrix m(2);`
...
An empty list of a certain dimension can be created much like a vector. `List L (3)` is equivalent to

```{r}
L<- vector(mode = "list", length = 3)
L
```
You can also use the `List::create()` function. `List L = List::create(v1, v2);` Creates list L from vector v1, v2
...
Vectors can be subset with square brackets using integer/numerical or logical indices or by using names `v[number]`
...
Note: When you access to vector elements using [] or () operator, the return type is not Vector itself but Vector::Proxy. Thus, it will cause compile error when you pass v[i] directly to some function, if the function only supports Vector type. To avoid compile error v[i] assign to new object or convert it to type T using as<T>().
...
```{r}
add_one(matrix(nrow=3, ncol=3, data=m1))
```
...
-   Shallow copies (See Rcpp for everyone) We saw examples of this with lists and dataframes where objects get linked and both update if one is updated. This can also happen with other Rcpp data types like vectors and matrices if you assign one variable equal to another variable.
